...
namespace ESFT.Common
{
    public abstract class CustomIniFile
    {
        ...
        {
            List<string> vStrings = new List<string>();
            this.ReadSection(Section, vStrings);
    ...
    public class IniFile : CustomIniFile
    {
        ...
        public IniFile(string AFileName)
            : base(AFileName)
        {
            if (!System.IO.Directory.Exists(AFileName.Substring(0, AFileName.LastIndexOf("\\"))))
            {
                System.IO.Directory.CreateDirectory(AFileName.Substring(0, AFileName.LastIndexOf("\\")));
            }

            if (!System.IO.File.Exists(AFileName))
            {
                System.IO.File.Create(AFileName);
            }
        }
        ...
        {
            //Ident = DESEncrypt.Encrypt(Ident);
            //Default = DESEncrypt.Encrypt(Default);
            byte[] vBuffer = new byte[2048];
            uint vCount = GetPrivateProfileString(Section, Ident, Default, vBuffer, (uint)vBuffer.Length, FileName);
        ...
        public override bool WriteString(string Section, string Ident, string Value)
        {
            ...
            return WritePrivateProfileString(Section, Ident, Value, this.FileName);
        }
        ...
        public override bool ReadSectionValues(string Section, List<string> Strings)
        {
            ...
            if (!this.ReadSection(Section, vIdentList))
            {
                return false;
            }
            ...
            {
                Strings.Add(string.Format("{0}={1}", vIdent, this.ReadString(Section, vIdent, string.Empty)));
            }
        ...
        public override bool ReadSection(string Section, List<string> Strings)
        {
            ...
            uint vLength = GetPrivateProfileString(Section, null, null, vBuffer, (uint)vBuffer.Length, FileName);
            int j = 0;
        ...
        {
            byte[] vBuffer = new byte[16384];
            uint vLength = GetPrivateProfileString(null, null, null, vBuffer, (uint)vBuffer.Length, FileName);
            int j = 0;
            for (int i = 0; i < vLength; i++)
            {
                if (vBuffer[i] == 0)
        ...
        {
            return WritePrivateProfileString(Section, null, null, this.FileName);
        }
        ...
        {
            //Ident = DESEncrypt.Encrypt(Ident);
            return WritePrivateProfileString(Section, Ident, null, this.FileName);
        }
        ...
        {
            return WritePrivateProfileString(null, null, null, this.FileName);
        }
    }
}
