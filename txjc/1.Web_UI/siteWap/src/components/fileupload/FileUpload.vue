<template>
  <div class="bg-white">
    <!--
                <img class="file-uploads-img" v-for="img in serverFiles" :src="img" />
                -->
    <div v-for="img in serverFiles" class="file-uploads-img b1">
      <img :src="imgUrl + img" />
    </div>
    <label class="file-uploads file-uploads-btn p10 b1" caria-label="Left Align" :class="mode === 'html5' ? 'file-uploads-html5' : 'file-uploads-html4'">
      <span class="plus-icon file-uploads-title glyphicon glyphicon glyphicon-plus" aria-hidden="true" v-html="title"></span>
      <slot></slot>
      <input-file>
        </input-file />
    </label>
        ...
        switch (e.type) {
          ...
          default:
            if (xhr.status >= 500) {
              ...
              file.success = true;
              me.progress = '';
              var rt = JSON.parse(e.currentTarget.responseText);

              if (rt && rt.ResponseID == 0 && rt.Data && rt.Data.length > 0) {
                rt.Data.forEach(function (item) {
                  me.serverFiles.push(item);
                });
              }
              else {
                me.showTips(e.Message, 'error');
              }
            }
        }
        file.active = false;
