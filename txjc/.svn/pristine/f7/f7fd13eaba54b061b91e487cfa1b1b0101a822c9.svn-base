                ...
                <div v-for="item0 of dataIn.table" class="order-item">
                    ...
                    <div class="order-summary">
                        ...
                        <div v-if="item0.Message != ''">
                            <span class="text-bold">卖家留言：</span>
                            <span class="c-color66">{{item0.Message}}</span>
                        </div>
                        <div>
                            共
                            <span class="text-bold c-red">{{item0.SumProCount}}</span>件商品，合计
                        ...
                        <div>
                            ...
                            <input v-on:click="btnOrderWriteOff_Click(item0);" v-if="item0.OrderStatus == 11" type="button" value="审核凭证" class="btn-small" />
                            ...
                            <input v-on:click="btnChangePrice_Click(item0)" v-if="item0.OrderStatus == 1" type="button" value="改价" class="btn-small" />
                        </div>
                    </div>
                </div>
            </div>
...
export default {
    ...
    methods: {
        ...
        nextPage_Click: function () {
            var me = this;
            me.PageIndex++;
            me.getOrderInfo(me.PageIndex, me.orderStatus);
        },
        getOrderInfo: function (iPageIndex, iOrderStatus) {
            ...
            me.fetchData({
                ...
                callback: function (data) {
                    ...
                    if (parseInt(me.PageIndex) == parseInt(data.TotalPages)) {
                        me.ShowMore = false;
                    }
                    else {
                        me.ShowMore = true;
        ...
        btnOrderWriteOff_Click: function (order) {
            var me = this;

            dialog({
                title: '提示',
                content: '请确认收到买家线下支付的款项后再通过审核，继续审核吗？',
                ok: function () {
                    var para = {};
                    para.OrderCode = order.OrderCode;

                    this.fetchData({
                        cmd: '/api/ProductOrder/OrderWriteOff',
                        para: para,
                        callback: function (data) {
                            me.getOrderInfo(1, 2);
                        }
        ...
        btnChangePrice_Click: function (order) {
            this.$router.push({ path: '/omp/' + order.OrderCode });
        },
